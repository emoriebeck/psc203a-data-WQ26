...
1. In the code chunk below, complete the following:
    ...
    - Use the `load()` and `url()` functions to download the `df_school_all` dataframe from the url: `https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/05-assignments/02-ps2/recruit_school_allvars.RData`
...
```{r}
library(tidyverse)
load(url("https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/05-assignments/02-ps2/recruit_school_allvars.RData"))
```
...
```{r}
cls <- c("name", "state_code", "city", "school_type", "total_visits", "med_inc", "pct_white", "pct_black", "pct_hispanic", "pct_asian", "pct_amerindian")
...
# bonus: combined!
df_school_all %>%
  arrange(school_type, total_visits) %>%
...
df_school_all %>%
  mutate(
    ca_school = if_else(state_code == "CA", 1, 0)
  ) %>% 
  count(ca_school)
...
df_school_all %>%
  mutate(
    ca_pub_school = ifelse(state_code == "CA" & school_type == "public", 1, 0)
  ) %>% 
  count(ca_pub_school)
```
...
```{r}
cls <- c("name", "pct_black", "pct_hispanic", "pct_amerindian")
df_race <- df_school_all %>%
  select(cls)

df_race %>% 
  filter(!is.na(pct_black) | !is.na(pct_hispanic) | !is.na(pct_amerindian)) %>%
  count(pct_black, pct_hispanic, pct_amerindian)

cls <- c("pct_black", "pct_hispanic", "pct_amerindian")
df_race <- df_race %>%
  rowwise() %>%
  mutate(pct_bl_hisp_nat = sum(c_across(all_of(cls)))) %>%
  ungroup() %>%
...
df_school_all %>%
  ...
  summarize(
    total_students = sum(total_students)
    , total_students_miss = sum(total_students[is.na(med_inc)])
    , av_med_inc = mean(med_inc, na.rm = T)
    , max_med_inc = max(med_inc, na.rm = T)
    , min_med_inc = min(med_inc, na.rm = T)
  )
```
