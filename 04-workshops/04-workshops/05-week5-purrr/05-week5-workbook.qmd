...
df1 <- tibble(
  ...
  unnest(data) %>%
  select(ID, count, all_of(old.names)) # select columns from codebook in loaded data   names  
head(df1, 6)
```
...
```{r}
summary(nested_mods$model[[1]])
fixef(nested_mods$model[[1]])
ranef(nested_mods$model[[1]])
VarCorr(nested_mods$model[[1]])

broom::tidy(nested_mods$model[[1]])
