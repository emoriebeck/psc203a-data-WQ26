...
-   Today, though, we'll focus on the `purrr::map()` family of functions
...
-   For character, we'd call `map_chr()`:
...
::: fragment
```{r get data}
data_source <- "https://github.com/emoriebeck/psc203a-data-WQ26/raw/refs/heads/main/04-workshops/05-week5-purrr/week-5.zip"
...
:::: column 
```{r, eval = F}
#| code-line-numbers: "|1-3|4-8|9-16"
mod_tab <- nested_mods %>%
  select(trait, tidy) %>%
  unnest(tidy) %>%
  pivot_wider(
...
```{r}
#| code-line-numbers: "|5-7|8|9-10|12|13|1-2,14"
hdr <- c(1, rep(3, 5))
