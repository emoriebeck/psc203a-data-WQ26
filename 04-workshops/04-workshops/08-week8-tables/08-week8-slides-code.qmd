...
# APA Tables {background-color="black"}
...
## [Tables in R]{style="color:white;"} {.smaller background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/green-top.png" background-size="100%"}
...
## [Important Tools]{style="color:white;"} {background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/green-top.png" background-size="100%"}
...
## [Data]{style="color:white;"} {.smaller background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/green-top.png" background-size="100%"}
...
## [Data]{style="color:white;"} {background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/green-top.png" background-size="100%"}

```{r}
(gsoep <- read_csv("week8-data.csv"))
```

# Basic: One DV, Multiple Model Terms {background-color="black"}

We'll start with a basic case, predicting who has a child from personality traits.
...
In other woRData, we're predicting the log odds of having a child from a linear combination of predictor variables.

## [Set up the data frame]{style="color:white;"} {background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/blue-top.png" background-size="100%"}

First, we'll use some of what we learned in the `purrr` workshop to set ourselves up to be able to create these tables easily:

-   Choose target target personality + outcome combinations using `filter()`
-   Using `group_by()` and `nest()` to create nested data frames
-   Using `mutate()` and `map()` to create new objects.
...
## [Set up the data frame]{style="color:white;"} {.smaller background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/blue-top.png" background-size="100%"}

-   We now have a data frame with 3 columns (Outcome, Trait, and data) and 4 rows.

    -   The data column is of class list, meaning it's a "list column" that contains a `tibble` in each cell.
    -   This means that we can use `purrr` functions to run operations on each of these data frames individually but without having to copy and paste the same operation multiple times for each model we want to run.
...
## [Run Models]{style="color:white;"} {background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/blue-top.png" background-size="100%"}

-   To run the models, I like to write short functions that are easier to read than including a local function within a call to `purrr::map()`.
-   Here, we're just going to write a simple function to predict child birth from personality.

::: fragment
```{r}
mod1_fun <- function(d, trait, outcome){
  d$o_value <- factor(d$o_value)
  m <- glm(o_value ~ p_value, data = d, family = binomial(link = "logit"))
  # save(m, file = "results/models/%s-%s.RData")
  return(m)
}

gsoep_nested1 <- gsoep_nested1 %>%
  mutate(m = pmap(list(data, Trait, Outcome), mod1_fun))
```
:::
...
## [Get Key Terms]{style="color:white;"} {background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/blue-top.png" background-size="100%"}

-   Now that we have the models, we want to get our key terms.
-   I'm a big fan of using the function `tidy` from the `broom` package to do this because
    -   It plays nicely with `purrr`
    -   It will give us confidence intervals, which I generally prefer over p-values and standard errors because I find them more informative.

## [Get Key Terms]{style="color:white;"} {background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/blue-top.png" background-size="100%"}

-   This is a local function, meaning that I've used the notation `~`function(., arguments). The tilda tells `R` we want a local function, and the `.` tells R to use the mapped `m` column as the function input.
...
gsoep_nested1
gsoep_nested1$tidy[[1]]
```
:::

-   Now we have a fifth column, which is a list column called `tidy` that contains a `tibble`, just like the `data` column.

## [Creating a Table]{style="color:white;"} {background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/blue-top.png" background-size="100%"}

Now we are finally ready to create a table! I'm going to use `kable` + `kableExtra` to do this in steps.

1.  Drop drop the `data` and `m` columns because they've done their work for now.
2.  `unnest()` the `tidy` column from our data frame. Before doing so, we will
...
::: column
```{r, echo = F}
tidy1 %>% print(n = 6, width = 50)
```
:::
:::::
::::::

## [Creating a Table]{style="color:white;"} {.smaller background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/blue-top.png" background-size="100%"}
...
-   **What is the desired final structure for the table?**
    -   I'd like columns for Trait, estimate (b), and confidence intervals (CI) formatted to two decimal places and bolded if significant.
    -   I'd also like a span header denoting that the outcome measure is child birth.
...
## [Kabling a Table]{style="color:white;"} {.smaller background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/red-top.png" background-size="100%"}

Now let's create the table using `knitr::kable()`

```{r}
kable(tidy1)
```

## [Kabling a Table]{style="color:white;"} {background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/red-top.png" background-size="100%"}

:::::: columns
::: column
-   It will automatically generate the html code needed to create a table.
-   But if we look closely at the code, it gives us some gobbledigook where we inputted html, so we need a way around that.
-   I'm also going to throw in `kable_styling(full_width = F)` from the `kableExtra` to make the formatted table print in your Viewer.
:::
...
tidy1 %>%
  mutate(Trait = factor(Trait, levels = p_names$old, labels = p_names$new)) %>%
  arrange(Trait) %>%
  kable(.
        , format = "html"
        , escape = F
...
# save(tab, file = "results/tables/tab-1-main-effects.html")
```
:::
...
(tab <- tidy1 %>%
  ...
  kable_classic(full_width = F, html_font = "Times", font_size = 20)%>%
  add_header_above(c(" " = 1, "Birth of a Child" = 2)) %>%
  add_footnote(label = "Note. Bold values indicate terms whose confidence intervals did not overlap with 0", notation = "none"))
```
:::
:::::
...
```{r}
save_kable(tab, file = "tab-1-ex-1.html")
```

## [A Quick Note: HTML v. LaTeX]{style="color:white;"} {.smaller background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/red-top.png" background-size="100%"}
...
# Extracting terms from `lavaan` {background-color="#016295"}
...
## [Data]{style="color:white;"} {background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/mykonos-top.png" background-size="100%"}
...
```{r}
remove_neg <- function(x) {x[x < 0] <- NA; x}
recode_le <- function(x) mapvalues(x, seq(-7,1), c(rep(NA, 5), 0, NA, NA, 1), warn_missing = F)
rev_code <- c("Big5__A_coarse", "Big5__C_lazy", "Big5__E_reserved", "Big5__N_dealStress")

gsoep2 <- read_csv("week8-data-2.csv") %>%
   mutate_at(vars(matches("Big5")), remove_neg) %>%
   mutate_at(vars(matches("LifeEvent")), recode_le) %>%
   mutate_at(vars(all_of(rev_code)), ~8-.)
gsoep2 %>% print(n = 6)
```
...
gsoep2_long %>% print(n = 6)
```

## [Data Cleaning]{style="color:white;"} {.smaller background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/mykonos-top.png" background-size="100%"}
...
gsoep2_lavaan %>% print(width = 50)
```
:::
:::::
::::::

## [Second-Order Latent Growth Model]{style="color:white;"} {.smaller background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/mykonos-top.png" background-size="100%"}

-   Because of how we set up the data, we only have to write this out once.
-   SEM b syntax can get really long, making it both time consuming and error-prone to write out different models for each predictor
...
## [Second-Order Latent Growth Model]{style="color:white;"} {background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/mykonos-top.png" background-size="100%"}
...
lavaan_fun <- function(d, trait){
  ...
  # save(m, file = sprintf("results/models/%s.RData", trait))
  return(m)
}
```
...
## [Extracting Results]{style="color:white;"} {.smaller background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/mykonos-top.png" background-size="100%"}
...
extract_fun <- function(m, trait){
  p <- parameterEstimates(m) %>%
    data.frame()
  # if you prefer standardized
  # p <- standardizedSolution(m) %>%
  #   data.frame()
  # save(p, file = sprintf("results/summary/%s.RData", trait))
  p %>%
...
## [Extracting Results]{style="color:white;"} {background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/mykonos-top.png" background-size="100%"}
...
gsoep_nested2 <- gsoep_nested2 %>%
  mutate(summary = map2(m, trait, extract_fun))
gsoep_nested2
```

## [Formatting Results]{style="color:white;"} {background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/mykonos-top.png" background-size="100%"}
...
```{r}
#| code-line-numbers: "6-15|8|9|10-12|13|14"
...
## [Kabling the Table]{style="color:white;"} {background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/red-top.png" background-size="100%"}
...
```{r, eval = F}
#| code-line-numbers: "|6-11|7|8|9,4|10,3|12|13,1-2|14"
hdr <- c(1, rep(2,5))
names(hdr) <- c(" ", "Extraversion", "Agreeableness", "Conscientiousness", "Neuroticism", "Openness")

cap <- "<strong>Table 2</strong><br><em>Big Five Personality Trait Trajectories from Latent Growth Models</em>"

cn <- c("Term", rep(c("<em>est.</em>", "CI"), times = 5))
...
## [Kabling the Table]{style="color:white;"} {.smaller background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/red-top.png" background-size="100%"}
...
## [Slopes as Predictors]{style="color:white;"} {.smaller background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/blue-top.png" background-size="100%"}
...
## [Data Cleaning]{style="color:white;"} {background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/blue-top.png" background-size="100%"}
...
## [Model Setup]{style="color:white;"} {background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/blue-top.png" background-size="100%"}
...
## [Model Setup]{style="color:white;"} {.smaller background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/blue-top.png" background-size="100%"}
...
lavaan_fun <- function(d, trait, outcome){
  m <- growth(
    mod2
    , data = d
    , ordered = "le_value"
  ...
  # save(m, file = sprintf("/results/models/%s-%s.RData", trait, outcome))
  return(m)
}

gsoep_nested3 <- gsoep_nested3 %>%
  mutate(m = pmap(list(data, trait, le), lavaan_fun))
gsoep_nested3
# save(gsoep_nested3, file = "gsoep_nested3.RData")
```
:::

::: column
```{r, echo = F}
load("gsoep_nested3.RData")
gsoep_nested3
```
:::
:::::

## [Extract Results]{style="color:white;"} {background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/blue-top.png" background-size="100%"}
...
gsoep_nested3 <- gsoep_nested3 %>%
  mutate(summary = map(m, extract_fun))
gsoep_nested3 %>% print(width = 50)
```
:::
:::::

## [Formatting Results]{style="color:white;"} {.smaller background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/blue-top.png" background-size="100%"}
...
## [Formatting Results]{style="color:white;"} {background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/blue-top.png" background-size="100%"}
...
## [Kabling the Table]{style="color:white;"} {.smaller background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/blue-top.png" background-size="100%"}
...
```{r}
#| code-line-numbers: "|3-4,9-17|1-2,18|19|6-7,20-24"
hdr <- c(1, rep(2,5))
names(hdr) <- c(" ", "Extraversion", "Agreeableness", "Conscientiousness", "Neuroticism", "Openness")
cap <- "<strong>Table 3</strong><br><em>Big Five Personality Trait Trajectory Associations with Life Events from Latent Growth Models</em>"
cn <- c("Term", rep(c("<em>est.</em>", "CI"), times = 5))
...
## [Base R GLM]{style="color:white;"} {background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/green-top.png" background-size="100%"}
...
::: column
```{r, echo = F}
gsoep_nested4 %>% print(n = 10)
```
:::
:::::

## [Run Models]{style="color:white;"} {background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/green-top.png" background-size="100%"}

Now, we'll run the models predicting outcomes from personality x age interactions (and their lower order terms)

```{r}
mod2_fun <- function(d, trait, outcome){
  d$o_value <- factor(d$o_value)
  m <- glm(o_value ~ p_value*age, data = d, family = binomial(link = "logit"))
  # save(m, file = sprintf("results/models/%s-%s.RData", trait, outcome))
}

gsoep_nested4 <- gsoep_nested4 %>%
  mutate(m = pmap(list(data, Trait, Outcome), mod2_fun))
gsoep_nested4 %>% print(n = 6)
```

## [Generating Model Predictions]{style="color:white;"} {background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/green-top.png" background-size="100%"}
...
```{r}
#| code-line-numbers: "|1|9|2|3-6|8|9-11|12"
glm_pred_fun <- function(m){
  ...
  frame <- augment_columns(m, newdata = frame) %>%
    mutate(lower = .fitted - 1.96*.se.fit
           , upper = .fitted + 1.96*.se.fit
           ) %>%
    mutate_at(vars(.fitted, lower, upper), exp)
}
```
...
## [Plot the Predictions]{style="color:white;"} {background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/green-top.png" background-size="100%"}

-   One of the greatest strengths of list-columns and using `purrr` is what I think of as the data accordion.
    -   With list-columns, I can `unnest()` whatever level the models were estimated at and then reaggregate using whatever combinations / groups make the most sense.
-   So here, for example, we ran models for all Big Five trait (5) x life outcome (10) combinations.
-   But having 50 separate plots would be a little silly, so we'd much rather create one for each outcome.
-   But I'm not going to write that code 10 times because that'd be a waste. So instead if I reaggregate the data, I can then use a function + `purrr` to generate each of the plots.

## [Plot the Predictions]{style="color:white;"} {.smaller background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/green-top.png" background-size="100%"}
...
plot_fun <- function(d, outcome){
  p <- d %>%
    ...
    ggplot(aes(x = p_value, y = .fitted, color = age_fac)) +
    geom_ribbon(
    ...
    my_theme()
  # ggsave(p, file = sprintf("results/plots/%s.png", outcome), width = 12, height = 6)
  # ggsave(p, file = sprintf("results/plots/%s.pdf", outcome), width = 12, height = 6)
}
```
:::
...
## [Lavaan Trajectories]{style="color:white;"} {.smaller background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/purple-top.png" background-size="100%"}
...
## [Lavaan Trajectories]{style="color:white;"} {background-color="white" background-image="https://github.com/emoriebeck/psc203a-data-WQ26/raw/main/psc203a-data-WQ26/images/purple-top.png" background-size="100%"}

```{r, eval = F}
gsoep_nested2 <- gsoep_nested2 %>%
  mutate(pred = map(m, lavaan_pred_fun))
gsoep_nested2
# save(gsoep_nested2, file = "gsoep_nested2.RData")
```

```{r, echo = F}
load("gsoep_nested2.RData")
gsoep_nested2
```
